...
from blacksmith import AsyncLoggingMiddleware, __version__
...
async def test_logging_middleware_no_errors(
    echo_middleware: AsyncMiddleware,
    req: HTTPRequest,
    dummy_timeout: HTTPTimeout,
    ...
    middleware = AsyncLoggingMiddleware(log)
    next = middleware(echo_middleware)
    await next(req, "dummy", "/dummies/{name}", dummy_timeout)
...
async def test_logging_middleware_log_response_disabled(
    echo_middleware: AsyncMiddleware,
    req: HTTPRequest,
    dummy_timeout: HTTPTimeout,
    logger: type[logging.Logger],
    ...
    log = logger("", level=level)
    middleware = AsyncLoggingMiddleware(log, log_response=False)
    next = middleware(echo_middleware)
    await next(req, "dummy", "/dummies/{name}", dummy_timeout)
...
async def test_logging_middleware_errors(
    invalid_middleware: AsyncMiddleware,
    req: HTTPRequest,
    dummy_timeout: HTTPTimeout,
    logger: type[logging.Logger],
    ...
    with pytest.raises(HTTPError):
        await next(req, "dummy", "/", dummy_timeout)
...
async def test_logging_middleware_broken(
    broken_middleware: AsyncMiddleware,
    req: HTTPRequest,
    dummy_timeout: HTTPTimeout,
    logger: type[logging.Logger],
    level: int,
