...
from blacksmith.service._async.adapters.httpx import AsyncHttpxTransport, build_headers
...
async def test_query_http_with_proxy(patch: Any) -> None:
    # Test that using a proxy will set the proper cache key (also, coverage)
    transport = AsyncHttpxTransport(proxies={"http://": "http://proxy.mock"})
    resp = await transport(
