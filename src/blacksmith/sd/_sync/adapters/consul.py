...
) -> SyncClientFactory[ServiceRequest, Service, Any, Any]:
    sd = SyncStaticDiscovery({("consul", "v1"): endpoint})
    fact: SyncClientFactory[ServiceRequest, Service, Any, Any] = SyncClientFactory(
...
class SyncConsulDiscovery(SyncAbstractServiceDiscovery):
    ...
    def __init__(
        ...
        _client_factory: Callable[
            [Url, str], SyncClientFactory[ServiceRequest, Service, Any, Any]
        ] = blacksmith_cli,
    ) -> None:
